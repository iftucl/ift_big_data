...
def check_redis_config(host_name: str | None = None, redis_port: int | None = None):
    if not host_name:
        try:
            ...
            etl_mongo_logger.error("Could not find redis host env variable. please make sure REDIS_HOST name is correctly exported")
            raise
    if not redis_port:
        try:
            redis_port = os.environ["REDIS_PORT"]
        except KeyError:
            etl_mongo_logger.error("Could not find redis port env variable. please make sure REDIS_PORT name is correctly exported")
            raise
