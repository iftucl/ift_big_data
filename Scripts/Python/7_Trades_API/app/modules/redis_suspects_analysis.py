...
from modules.db_clients.redis_client import get_company_params
from app.api_models.api_responses.trade_suspect import TradeSuspect
from app.api_models.api_responses.trade_model import Trade


async def test_trades_peers(single_trade: Trade, confidence_level: float = 0.01) -> TradeSuspect:
    """
    ...
    if abs(z_score) > critical_value:
        lambro_logger.info(f"Reject H0: The stock's return ({single_trade.Symbol}) does NOT come from the same distribution.")
        ...
        lambro_logger.info(f"Fail to reject H0: The stock's return ({single_trade.Symbol}) may come from the same distribution.")
